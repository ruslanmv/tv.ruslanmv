...
import { fetchTodayEpisode, getApiBaseUrl } from "@/lib/api";
...
export default function Home() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const ep = await fetchTodayEpisode();
      setEpisode(ep);
      setLoading(false);
    })();
  }, []);
      ...
      <main className="flex-grow">
        ...
        <section className="bg-lightGray py-10 border-b border-gray-200">
          <div className="container mx-auto px-4">
            {!apiBase && (
              <div className="mb-6 bg-white border border-gray-200 rounded-lg shadow-soft p-4">
                <p className="text-sm text-muted">
                  <strong className="text-text">Backend not connected.</strong>{" "}
                  Set{" "}
                  <code className="px-1.5 py-0.5 bg-gray-100 rounded border border-gray-200">
                    NEXT_PUBLIC_API_URL
                  </code>{" "}
                  in Vercel to show live episodes.
                </p>
              </div>
            )}
            ...
            {apiBase && !episode && (
              <div className="mb-6 bg-white border border-gray-200 rounded-lg shadow-soft p-4">
                <p className="text-sm text-muted">
                  <strong className="text-text">No episode returned.</strong>{" "}
                  Your API URL is set, but{" "}
                  <code className="px-1.5 py-0.5 bg-gray-100 rounded border border-gray-200">
                    /api/v1/episodes/today
                  </code>{" "}
                  didn't return data (or CORS blocked it).
                </p>
              </div>
            )}
