...
import ReactPlayer from "react-player"; // supports YouTube + file URLs
...
const TVPlayer: React.FC<TVPlayerProps> = ({ episode, autoPlay = true }) => {
  ...
  // ---------------------------------------------------------------------------
  // Resolve primary (YouTube) and fallback (local MP4) URLs
  // ---------------------------------------------------------------------------
  const apiBase =
  ...
  // Fallback: local file generated by scripts/generate_video.py and served by FastAPI
  const localVideoUrl = `${apiBase}/media/episode_video.mp4`;

  // ReactPlayer supports an array of URLs: it will try each in order.
  // 1) Prefer YouTube (production / uploaded episodes)
  // 2) Fallback to local MP4 (freshly generated, not yet uploaded)
  const playerUrl: string | string[] = youtubeUrl
    ? [youtubeUrl, localVideoUrl]
    : localVideoUrl;
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <div className="text-gray-500 text-xs font-mono">
                  ...
                  {formatTime(episode.duration || currentTime)}
                </div>
                <div className="text-gray-500 text-[10px] uppercase tracking-wide">
                  Source: {youtubeUrl ? "YouTube ▶ local fallback" : "Local"}
                </div>
              </div>
