...
def _split_sentences(text: str) -> List[str]:
    """Split script text into subtitle-sized chunks."""
    ...
    # If we ended up with one long blob, chunk by word length
...
def create_subtitle_file(script_text: str, duration: float) -> Path:
    ...
    with srt_path.open("w", encoding="utf-8") as f:
        for idx, sentence in enumerate(sentences, start=1):
            ...
            # Small overlap to avoid visual gaps
...
def build_ffmpeg_command(
    ...
    ticker_source = ticker_source[:400]  # keep it reasonably short

    # Avoid ffmpeg drawtext issues with quotes, colons, and newlines
    ticker_text = (
        ticker_source.replace("'", " ")
        .replace(":", " ")
        .replace("\n", " ")
        .replace("\r", " ")
    )
    ...
    if ticker_text:
        # Extra safety for backslashes and colons in drawtext
        safe_ticker = ticker_text.replace("\\", " ").replace(":", " ")

        filter_lines.append(
            f"[{current_label}]drawbox="
            # Use iw/ih (input width/height) for drawbox
            "x=0:y=ih-80:w=iw:h=80:color=0x020617@0.9:t=fill[tbk];"
    ...
    # Audio settings: force widely compatible AAC (48 kHz, stereo)
    audio_bitrate = os.getenv("VIDEO_AUDIO_BITRATE", "192k")
    audio_rate = os.getenv("VIDEO_AUDIO_RATE", "48000")
    audio_channels = os.getenv("VIDEO_AUDIO_CHANNELS", "2")
